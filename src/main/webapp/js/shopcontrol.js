...
app.directive('isNumber', function() {
	return {
		require : 'ngModel',
		link : function(scope) {
			scope.$watch('lst_shoplist.modifyPrice', function(newValue,
					oldValue) {
...
app.filter(
				...
				function(numberFilter) {
					...
					return function(inputNumber, currencySymbol,
							decimalSeparator, thousandsSeparator,
							decimalDigits, prefixWithSymbol) {
						if (isNumeric(inputNumber)) {
							// Default values for the optional arguments
							currencySymbol = (typeof currencySymbol === "undefined") ? "$"
									: currencySymbol;
							decimalSeparator = (typeof decimalSeparator === "undefined") ? "."
									: decimalSeparator;
							thousandsSeparator = (typeof thousandsSeparator === "undefined") ? ","
									: thousandsSeparator;
							decimalDigits = (typeof decimalDigits === "undefined" || !isNumeric(decimalDigits)) ? 2
									: decimalDigits;
							prefixWithSymbol = (typeof prefixWithSymbol === "undefined") ? true
									: prefixWithSymbol;
							...
							return (prefixWithSymbol ? currencySymbol + " "
									: "")
									+ result
									+ (prefixWithSymbol ? "" : " "
											+ currencySymbol);
...
app.controller("s1",
		function($scope) {
			...
			$scope.addItem = function() {
				$scope.shopingList.push({
					'itemId' : $scope.txtItemId,
				...
				$scope.netprices = $scope.netprices
						+ $scope.txtItemAspctTotalPrice;

				localStorage.setItem('itemStore', angular
						.toJson($scope.shopingList));
...
app.controller("s2",
		function($scope) {
			...
			if (lists) {
				...
				for (var i = 0; i < lists.length; i++) {
					...
					if (lists[i].itemId) {
						...
						if(lists[i].inBucket)
							{
							$scope.totalqtyinbucket = $scope.totalqtyinbucket  + lists[i].qty;
							}
			...
			$scope.inBucket = function(itemId, inbkt) {
				...
				localStorage.setItem('itemStore', angular
						.toJson($scope.shopingListInBucker));
			...
			if (lists[i].itemId) {
				...
				if (lists[i].inBucket == 1) {
					$scope.netprices = $scope.netprices
							+ lists[i].aspectTotalPrice;
					$scope.totalqty = $scope.totalqty + lists[i].qty;
					$scope.netactualprices = $scope.netactualprices
							+ lists[i].modifyTotalPrice;
					cnt++;
				}
			}
